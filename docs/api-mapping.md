...
|---|---|---|---|
| `GET /connection/` | `rate_down`, `rate_up` | `sensor` · `device_class: data_rate` · `MEASUREMENT` | En **octets/s**. HA sait convertir en Mbit/s côté frontend. |
| | `bandwidth_down`, `bandwidth_up` | `sensor` diagnostic | Débit de synchronisation négocié. Quasi statique. |
